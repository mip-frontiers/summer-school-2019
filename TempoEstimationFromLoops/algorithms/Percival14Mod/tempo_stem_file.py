...
from . import mar_collection

from . import onset_strength
from . import beat_period_detection
from . import accumulator_overall
...
def bpm_of_mf(defs, mf_filename, print_info=False):
    ...
    for dat in coll.data:
        ...
        if print_info:
            print("Accuracy: %.2f (%i/%i)" % (accuracy, good, i), end=' ')
            if not acc:
                print("\tground: %.2f\tdetected: %.2f" % (
                    bpm_ground, bpm_detect), end=' ')
                print(filename)
            print()
